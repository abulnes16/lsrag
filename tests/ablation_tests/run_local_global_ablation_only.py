...
async def run_local_global_update(filepath: str):
    print(f"\n--- Starting Local & Global update for {os.path.basename(filepath)} ---")
    ...
    light_config = {
        "working_dir": os.path.join(project_root, 'data', "corporate_lightrag_cache"),
        "llm_model": "phi3:mini",
        "embed_model": "mxbai-embed-large",
        "chunk_size": 1200,
        "chunk_overlap": 100,
    }
    print(f"Initializing LightRAG Service using working_dir: {light_config['working_dir']}...")
    rag_service = LightRAGService(light_config)
    await rag_service.rag.initialize_storages()
    
    print("Initializing RAGMetrics Service (requires OPENAI_API_KEY for GPT-4o-mini)...")
    metrics_service = RAGMetrics()
    
    # We want to re-run ONLY local and global.
    target_variants = ["local", "global"]
    ...
    for i, q_results in enumerate(results):
        ...
        for variant in target_variants:
            ...
            try:
                ...
                print(f"     Calculating metrics for {variant}...")
...
async def main():
    print("=====================================================")
    print(" L O C A L   &   G L O B A L   -   R E - E V A L U A T I O N")
    print("=====================================================\n")
    print("WARNING: This script will ONLY re-run queries and calculate metrics for Local & Global variants.")
    print("Existing results for NaiveRAG and Mix variants will be preserved.\n")
    
    base_dir = os.path.dirname(__file__)
    corp_file = os.path.join(base_dir, "evaluation_questions_corporate.json")
    
    if os.path.exists(corp_file):
        await run_local_global_update(corp_file)
    else:
